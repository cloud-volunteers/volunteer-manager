...
class Match:
        ...
        while len(queue) > 0:
            ...
            for node in cur_node.neighbors:
                if node == cur_node.parent:
                    ...
                    if len(cycle) % 2 == 1:
                        logger.debug('blossom: {}'.format(cycle))
    ...
    def maximum_matching(self):
        while len(self.freenodes) > 0:
            logger.debug('freenodes: {}'.format(self.freenodes))
            for node in self.freenodes:
                try:
                    path = self.find_augmenting_path(node)
                    logger.debug('augmenting path: {}'.format(path.nodes))
                ...
                logger.info('Tried all free nodes, no more augmenting path.')
                break
